...
void ConvolutionNode::apply(storage &grad){
    ...
    std::vector<std::vector<size_t>> slice_vector = {{static_cast<size_t>(b->tensor->data.shape[0]/2),grad.shape[0]-
    static_cast<size_t>((b->tensor->data.shape[0]-1)/2)+1,1},{static_cast<size_t>((b->tensor->data.shape[0]-1)/2),
    grad.shape[1]- static_cast<size_t>((b->tensor->data.shape[1]-1)/2)+1,1}};
    padded_grad.setslice(slice_vector,a->tensor->data);
