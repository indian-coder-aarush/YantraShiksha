...
std::vector<int> int_to_triplet(py::object& index, int& dim_size){
   int start = py::cast<int>(index), stop = py::cast<int>(index), step = 1;
   std::vector<int> a = {start};
   return a;
}


std::vector<std::vector<int>> slice_to_vector(py::tuple& slice, std::vector<int>& a_shape){
...
void Tensor::backward(){
   ...
   Tensor::Tensor(std::vector<int> &dim,double default_value):data(dim,default_value),Tensor_Node(std::make_shared<Node>()){
       Tensor_Node->tensor = std::make_shared<Tensor>(*this);
   }
   Tensor::Tensor():data(){}
...
Tensor convolution(Tensor &a, Tensor &b, int stride){
   ...
   c_node->a = a.Tensor_Node;
   c_node->b = b.Tensor_Node;
   c_node->strides = stride;
   std::shared_ptr<Tensor> tensor_c = std::make_shared<Tensor>(c);
