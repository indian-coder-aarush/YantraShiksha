...
class Tanitra:
   ...
   def __init__(self, data, track_gradient=True):
       self.data = np.array(data)  # Tensor data (stored as numpy array)
   ...
   def __add__(self, other):
       ...
       if self.track_gradient:
           ...
           new_tanitra.parents.append((self, lambda g: g*np.ones_like(other.data)))
           new_tanitra.parents.append((other, lambda g: g*np.ones_like(self.data)))
       return new_tanitra
   ...
   def dot(self,a):
       if not isinstance(a,Tanitra):
           a = Tanitra(a)
       new_Tanitra = Tanitra(np.dot(self.data,a.data))
   ...
   def backward(self,grad=None):
       if grad is None:
           grad = np.ones_like(self.data)  # Default gradient for final node
...
def sigmoid(data):
   if not isinstance(data,Tanitra):
       data = Tanitra(data)
   new_tanitra = Tanitra(1 / (1 + np.exp(-data.data)))
...
def relu(data):
   if not isinstance(data,Tanitra):
       data = Tanitra(data)
   new_tanitra = Tanitra(np.maximum(0,data.data))
   if data.track_gradient:
...
def square(data):
   if not isinstance(data,Tanitra):
       data = Tanitra(data)
   new_tanitra = Tanitra(np.square(data.data))
   if data.track_gradient:
...
def convolution2d(a,b,stride,padding_mode = None,pad_width = 0,constant_values = 0):
   if padding_mode is not None:
       a_padded= np.pad(a.data,pad_width = pad_width,mode = padding_mode,constant_values = constant_values)
   else:
       a_padded = a.data
   b_flipped = np.flip(b.data)
   ...
   if a.track_gradient or b.track_gradient:
       ...
       def gradient_function_a(grad):
           if stride<1:
               ...
               unsampled = np.zeros((H_grad, W_grad))
       ...
       def gradient_function_b(grad):
           unsampled = np.zeros((grad.shape[0]+(grad.shape[0]-1)*(stride-1),(grad.shape[1]+(grad.shape[1]-1)*
                                                                             (stride-1))))
           unsampled[::stride,::stride] = grad
...
def pooling2d(a,pool_size,stride,padding_mode= None,pad_width = None,constant_values = 0):
   ...
   new_tanitra_data = np.zeros((h,w))
   indices_list = []
   for i in range(h):
       for j in range(w):
           new_tanitra_data[i,j] = np.max(a.data[i:i+pool_size,j:j+pool_size])
           index = np.argmax(a.data[i:i + pool_size, j:j + pool_size])
   ...
   if a.track_gradient:
       ...
       def gradient_function(grad):
           unsampled = np.zeros_like(a_padded)
...
def softmax(a,axis=0):
   if not isinstance(a,Tanitra):
       a = Tanitra(a)
   maxed = a.data - np.max(a.data,axis = axis,keepdims = True)
   exponent = np.exp(maxed)
   sum = np.sum(exponent,axis = axis,keepdims = True)
   result = exponent/sum
...
def log(x):
   if not isinstance(x,Tanitra):
       x = Tanitra(x)
   x.data = np.clip(x.data, 1e-9, None)  # Avoid log(0)
   a = Tanitra(np.log(x.data))
   def grad_func(grad):
...
def cos(x):
   if not isinstance(x,Tanitra):
       x = Tanitra(x)
   a = Tanitra(np.cos(x.data))
   def grad_func(grad):
       return grad*-1*np.sin(x.data)
   if x.track_gradient:
       a.parents.append((x,grad_func))
   return a
...
def sin(x):
   if not isinstance(x,Tanitra):
       x = Tanitra(x)
   a = Tanitra(np.sin(x.data))
   def grad_func(grad):
       return grad*np.cos(x.data)
   if x.track_gradient:
       a.parents.append((x,grad_func))
   return a